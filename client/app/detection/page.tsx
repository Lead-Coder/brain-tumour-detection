...
export default function Detection() {
  ...
  const generatePDFReport = () => {
    ...
    doc.setFontSize(18)
    doc.text("Brain MRI Analysis Report", 105, 20, { align: "center" })
    ...
    doc.setFontSize(12)
    doc.text(
      `Result: ${result.status === "tumor" ? "Tumor Detected" : "No Tumor Detected"}`,
      20,
      65
    )

    if (result.status === "tumor" && result.tumorType) {
      doc.text(`Tumor Type: ${result.tumorType}`, 20, 75)
    }
    ...
    doc.setFontSize(14)
    doc.text("Disclaimer", 20, 105)

    doc.setFontSize(11)
    doc.text(
      "This report is generated by our NeuroDetect internal AI-based system and is intended for",
      20,
      115
    )
    doc.text(
      "informational purposes only. It should not be used as a substitute",
      20,
      122
    )
    doc.text(
              ...
              {!result && !analyzing && (
                <div className="flex gap-4">
                  <button
                    ...
                    className="flex-1 px-6 py-3 bg-primary cursor-pointer text-primary-foreground rounded-lg font-semibold">
                    Analyze Image
                  </button>
                  <button
                    onClick={handleReset}
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="flex-1 px-6 py-3 bg-card border rounded-lg cursor-pointer font-semibold flex items-center justify-center gap-2">
                      <FileText className="w-5 h-5" />
                      Generate Report (PDF)
                    </button>
                    <button
                      onClick={handleReset}
                      className="flex-1 px-6 py-3 bg-primary cursor-pointer text-primary-foreground hover:bg-sky-800 rounded-lg font-semibold">
